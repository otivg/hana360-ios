...
public class AudioPlayer implements MethodCallHandler, Player.Listener, MetadataOutput {
    ...
    // 360-PORT: custom Media3 AudioProcessor injected via DefaultAudioSink
    private AudioProcessor[] customAudioProcessors;
    ...
    private AudioEffect decodeAudioEffect(final Object json, int audioSessionId) {
        ...
        switch (type) {
        case "CustomAudioProcessor":
            return null; // handled by sink injection; not an audiofx effect
    ...
    private void setupCustomAudioProcessor() {
        customAudioProcessors = null;
        if (rawAudioEffects == null) return;
        for (Object raw : rawAudioEffects) {
            Map<?, ?> json = (Map<?, ?>) raw;
            if ("CustomAudioProcessor".equals(json.get("type"))) {
                String clsName = (String) json.get("processorClass");
                if (clsName == null) continue;
                try {
                    Class<?> cls = Class.forName(clsName);
                    Object inst = cls.getDeclaredConstructor().newInstance();
                    AudioProcessor proc = (AudioProcessor) cls.getMethod("create").invoke(inst);
                    customAudioProcessors = new AudioProcessor[] { proc };
                    android.util.Log.i("JustAudio", "CustomAudioProcessor created: " + clsName);
                } catch (Throwable t) {
                    android.util.Log.w("JustAudio", "CustomAudioProcessor class unavailable: " + clsName, t);
                    customAudioProcessors = null;
                }
            } else {
                android.util.Log.i("JustAudio", "CustomAudioProcessor: effect not found in pipeline");
            }
        }
    }
    ...
    private void ensurePlayerInitialized() {
        if (player == null) {
            setupCustomAudioProcessor();
            RenderersFactory renderersFactory = (eventHandler, videoListener, audioListener, textOutput, metadataOutput) -> {
                DefaultRenderersFactory factory = (customAudioProcessors != null && customAudioProcessors.length > 0)
                    ? new SpatialRenderersFactory(context, customAudioProcessors)
    ...
    /** 360-PORT: renderers factory that injects a custom DSP AudioProcessor chain. */
    private static class SpatialRenderersFactory extends DefaultRenderersFactory {
        private final AudioProcessor[] processors;

        SpatialRenderersFactory(Context context, AudioProcessor[] processors) {
            super(context);
        ...
        protected AudioSink buildAudioSink(
            ...
            // V3.2.6: AudioProcessor pipeline (ALL formats via reflection float chain patch)
            android.util.Log.i("JustAudio", "SpatialRenderersFactory: hybrid sink (proc=" + (processors != null ? processors.length : 0) + ")");
            DefaultAudioSink.Builder b = new DefaultAudioSink.Builder()
                    .setAudioCapabilities(
                            androidx.media3.exoplayer.audio.AudioCapabilities.DEFAULT_AUDIO_CAPABILITIES)
                    .setAudioOffloadSupportProvider(
                            (format, audioAttributes) -> androidx.media3.exoplayer.audio.AudioOffloadSupport.DEFAULT_UNSUPPORTED)
                    .setEnableFloatOutput(true);
            if (processors != null && processors.length > 0) {
                b.setAudioProcessors(processors);
            }
            AudioSink sink = b.build();
            // V3.2.6: 反射注入 AudioProcessor 到 float 链 (绕过 goto 101 缺陷)
            if (processors != null && processors.length > 0) {
                try {
                    java.lang.reflect.Field f = sink.getClass().getDeclaredField("toFloatPcmAvailableAudioProcessors");
                    f.setAccessible(true);
                    com.google.common.collect.ImmutableList old = (com.google.common.collect.ImmutableList) f.get(sink);
                    com.google.common.collect.ImmutableList.Builder lb = com.google.common.collect.ImmutableList.builder();
                    lb.addAll(old);
                    for (AudioProcessor p : processors) lb.add(p);
                    f.set(sink, lb.build());
                    android.util.Log.i("JustAudio", "float chain patched: " + old.size() + " → " + lb.build().size() + " processors");
                } catch (Throwable t) {
                    android.util.Log.w("JustAudio", "float chain patch failed: " + t);
                }
            }
            return new RawPcmForwardingAudioSink(sink);
        }
    }

    /**
     * V3.1.2: 把解码输出的 PCM 格式 MIME 统一改写成 "audio/raw"。
     *
     * Media3 DefaultAudioSink.configure() 的分支:
     *   if ("audio/raw".equals(format.sampleMimeType)) → PCM 处理链 (AudioProcessor 全生效)
     *   else → 空 pipeline 直通 (所有 AudioProcessor 被绕过)
     *
     * FLAC 解码器 (c2.android.flac.decoder) 输出 float PCM 但 MIME 保留 "audio/flac"
     * → 高解析音频 (48k/96k) 被 Media3 判定为非 PCM → 直通绕过 DSP。
     * 这里仅当 pcmEncoding 确为线性 PCM 时才改写 (passthrough 压缩格式不受影响)。
     */
    private static class RawPcmForwardingAudioSink extends ForwardingAudioSink {
        // V3.1.6: 拦截 audio 数据在 handleBuffer() 做 DSP, 绕开 Media3 pipeline float 路径缺陷
        private int sinkSampleRate = 44100;
        private int sinkChannels = 2;
        private int sinkEncoding = C.ENCODING_PCM_16BIT;
        private int sinkBytesPerFrame = 4;
        private int hbCount = 0;

        RawPcmForwardingAudioSink(AudioSink sink) {
            super(sink);
        }

        @Override
        public void configure(Format inputFormat, int specifiedBufferSize, int[] outputChannels)
                throws androidx.media3.exoplayer.audio.AudioSink.ConfigurationException {
            android.util.Log.i("JustAudio", "SinkConfigure: mime=" + inputFormat.sampleMimeType
                    + " rate=" + inputFormat.sampleRate + " ch=" + inputFormat.channelCount
                    + " enc=" + inputFormat.pcmEncoding);
            // 适配 DSP 配置
            if (inputFormat.pcmEncoding != C.ENCODING_INVALID
                    && Util.isEncodingLinearPcm(inputFormat.pcmEncoding)) {
                int prevRate = sinkSampleRate;
                sinkSampleRate = inputFormat.sampleRate;
                sinkChannels = inputFormat.channelCount;
                sinkEncoding = inputFormat.pcmEncoding;
                sinkBytesPerFrame = Util.getPcmFrameSize(sinkEncoding, sinkChannels);
                android.util.Log.i("JustAudio", "DSP config: " + sinkSampleRate + "Hz/"
                        + sinkChannels + "ch bpf=" + sinkBytesPerFrame + " enc=" + sinkEncoding);
                // V3.1.8: 采样率变化时重配 DMA 引擎 (之前缺失, 导致 DSP 恒跑 48kHz → 96k/44.1k 全失真)
                if (prevRate != sinkSampleRate && sDspBridgeInst != null) {
                    try {
                        java.lang.reflect.Method ofc = Class.forName(
                                "com.hanamimi.spatial.SpatialDspBridge")
                                .getMethod("onFormatChanged", int.class, int.class);
                        ofc.invoke(sDspBridgeInst, sinkSampleRate, sinkChannels);
                        android.util.Log.i("JustAudio", "DSP onFormatChanged: " + sinkSampleRate + "Hz");
                    } catch (Throwable t) {
                        android.util.Log.e("JustAudio", "DSP onFormatChanged fail: " + t);
                    }
                }
            }
            // MIME 改写不再需要 (仅对 PCM 容器格式做兼容)
            boolean pcmContainer = isPcmContainerMime(inputFormat.sampleMimeType);
            boolean linearPcm = Util.isEncodingLinearPcm(inputFormat.pcmEncoding);
            if (inputFormat.sampleMimeType != null
                    && !"audio/raw".equals(inputFormat.sampleMimeType)
                    && (pcmContainer || linearPcm)) {
                inputFormat = inputFormat.buildUpon().setSampleMimeType("audio/raw").build();
            }
            super.configure(inputFormat, specifiedBufferSize, outputChannels);
        }

        @Override
        public boolean handleBuffer(java.nio.ByteBuffer buffer, long presentationTimeUs, int flags)
                throws androidx.media3.exoplayer.audio.AudioSink.InitializationException,
                       androidx.media3.exoplayer.audio.AudioSink.WriteException {
            // V3.2.6: 全部通过 AudioProcessor pipeline (float 路径已反射修复)
            return super.handleBuffer(buffer, presentationTimeUs, flags);
        }

        // === V3.1.7: 反射调用 SpatialDspBridge (app 模块) — 修复 Kotlin object/Boolean 反射 ===
        // Kotlin `object SpatialDspBridge` → Java singleton via `INSTANCE` field
        // Kotlin `@Volatile var bypass: Boolean` → getter `getBypass()`, backing `isBypass`
        private static Object sDspBridgeInst;
        private static java.lang.reflect.Method sDspGetBypass;
        private static java.lang.reflect.Method sDspProcessDirect;
        private static java.lang.reflect.Method sDspProcessFloats;
        static {
            try {
                Class<?> bridge = Class.forName("com.hanamimi.spatial.SpatialDspBridge");
                sDspBridgeInst = bridge.getField("INSTANCE").get(null);
                sDspGetBypass = bridge.getMethod("getBypass");
                sDspProcessDirect = bridge.getMethod("processDirect",
                        java.nio.ByteBuffer.class, java.nio.ByteBuffer.class, int.class);
                sDspProcessFloats = bridge.getMethod("processFloats",
                        float[].class, int.class);
                android.util.Log.i("JustAudio", "DSP reflection wired");
                // 预取 onFormatChanged (静态调用, 设置到 companion)
                java.lang.reflect.Method ofc = bridge.getMethod("onFormatChanged", int.class, int.class);
                try { ofc.invoke(sDspBridgeInst, 48000, 2); } catch (Throwable t) {}
                android.util.Log.i("JustAudio", "DSP onFormatChanged prewired");
            } catch (Throwable t) {
                android.util.Log.e("JustAudio", "DSP reflection fail: " + t);
            }
        }
        private static boolean isDspBypass() {
            try {
                return sDspGetBypass != null && (Boolean)sDspGetBypass.invoke(sDspBridgeInst);
            } catch (Throwable t) { return true; }
        }
        private static void invokeDspProcessDirect(java.nio.ByteBuffer in, java.nio.ByteBuffer out, int frames) {
            try {
                if (sDspProcessDirect != null) sDspProcessDirect.invoke(sDspBridgeInst, in, out, frames);
            } catch (Throwable t) { /* fail-safe */ }
        }
        private static void invokeDspProcessFloats(float[] buf, int frames) {
            try {
                if (sDspProcessFloats != null) sDspProcessFloats.invoke(sDspBridgeInst, buf, frames);
            } catch (Throwable t) { /* fail-safe */ }
        }

        /** V3.1.3: 解码输出为线性 PCM 的容器格式白名单 (passthrough 压缩格式不在此列)。 */
        private static boolean isPcmContainerMime(String mime) {
            ...
            switch (mime) {
                ...
                case "audio/ogg":
                case "audio/3gpp":
                case "audio/mp4a-latm":   // 某些设备 AAC 解码输出保留此 MIME
                case "audio/mpeg":        // 某些设备 MP3 解码输出保留此 MIME
                    return true;
                default:
                    return false;
            }
