...
/// 360-PORT: custom Media3 AudioProcessor effect (sink-level DSP injection)
class CustomAudioProcessorMessage extends AudioEffectMessage {
  final String processorClass;

  CustomAudioProcessorMessage({
    required super.enabled,
    required this.processorClass,
  });

  @override
  Map<dynamic, dynamic> toMap() => <dynamic, dynamic>{
        'type': 'CustomAudioProcessor',
        'enabled': enabled,
        'processorClass': processorClass,
      };
}

/// 360-PORT: mounts a custom Media3 [AudioProcessor] (from the host app) into
/// the ExoPlayer audio chain. Enable/disable + parameters are managed by the
/// host app's own MethodChannel (SpatialDspController); this effect only
/// declares the processor class name to the patched plugin.
class CustomAudioProcessorEffect extends AudioEffect with AndroidAudioEffect {
  CustomAudioProcessorEffect(this.processorClass);

  /// Fully-qualified class name of an [AudioProcessor.Factory] implementation
  /// provided by the host app (e.g. com.hanamimi.spatial.SpatialAudioProcessorFactory).
  final String processorClass;

  @override
  String get _type => 'CustomAudioProcessor';

  @override
  AudioEffectMessage _toMessage() =>
      CustomAudioProcessorMessage(enabled: enabled, processorClass: processorClass);
}
