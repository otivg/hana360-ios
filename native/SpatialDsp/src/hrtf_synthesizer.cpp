...
#ifdef __ANDROID__
// Android 资产加载路径 (AAssetManager) — 仅 Android 编译
...
void HrtfSynthesizer::getCached(float azimuth, float* hrirLeft, float* hrirRight) {
    ...
    // V9.5: 环绕声道（DTS 5.1 LS/RS ±110°）允许 clamp 到 ±80° 取最近 HRIR，
    // 不再直接返回全 0（否则 360 ON 时环绕声道完全丢失 → 空间感塌陷）。
    // find_az 内部已 clamp 到 [-80°, +80°]。
    int ai = find_az(azimuth);
